...
router.get('/', async (req, res) => {
    try {
        const [ventas] = await pool.query(`
            SELECT 
                v.*, 
                v.vennumero_factura AS vennumero, 
                c.clinombre, 
                c.clicidruc, 
                u.usuusuario
...
// CREAR VENTA, FIFO, REGISTRO AL KARDEX
        ...
        for (const item of detalles) {
            ...
            while (cantPendiente > 0) {
                const [lotes] = await connection.query(`
                    SELECT lotid, lotcantidad_actual, lotfecha_vencimiento, provid 
                    FROM lotes 
                    WHERE prodid = ? 
                      AND lotcantidad_actual > 0 
                      AND lotactivo = 1
                      AND (lotfecha_vencimiento >= CURDATE() OR lotfecha_vencimiento IS NULL)
                    ORDER BY lotfecha_vencimiento ASC, lotid ASC
                    LIMIT 1
                `, [item.prodid]);

                let lotidUsado = null;
                let providUsado = null;
                let cantDescontada = cantPendiente;

                if (lotes.length > 0) {
                    const lote = lotes[0];
                    lotidUsado = lote.lotid;
                    providUsado = lote.provid;
                    ...
                    throw new Error(`No es posible vender ${producto.prodnombre}. Los lotes disponibles están caducados o no hay lotes vigentes.`);
                }
            ...
            const providParaKardex = detallesInsertados[0]?.provid || null;
            const lotidParaKardex = detallesInsertados[0]?.lotid || null;

            await connection.query(`
                INSERT INTO kardex (prodid, provid, lotid, kartipo, karcantidad, karsaldo_anterior, 
                        karsaldo_actual, karref_documento, karobservacion, usuid)
                         VALUES (?, ?, ?, 'VENTA', ?, ?, ?, ?, ?, ?)
            `, [item.prodid, providParaKardex, lotidParaKardex, item.cantidad, saldoAnterior, saldoNuevo, numFactura, 'Venta', usuid]);
        }
    ...
    try {
        ...
        for (const det of detalles) {
            ...
            const [loteData] = await connection.query(
                'SELECT provid FROM lotes WHERE lotid = ?',
                [det.lotid]
            );
            const provid = loteData.length > 0 ? loteData[0].provid : null;

            await connection.query(`
    INSERT INTO kardex (prodid, provid, lotid, kartipo, karcantidad, karsaldo_anterior,
                        karsaldo_actual, karref_documento, karobservacion, usuid)
    VALUES (?, ?, ?, 'DEVOLUCION', ?, ?, ?, ?, ?, ?)
`, [det.prodid, provid, det.lotid, det.vdetcantidad, saldoAnterior, saldoNuevo,
